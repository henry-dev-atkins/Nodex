...
- Internal backend history reconstruction and response-item projection are now separated into dedicated services; session lifecycle/reopen orchestration, conversation CRUD orchestration, turn execution/interruption orchestration, turn-record persistence/normalization, thread start/resume payload shaping, schema contract verification, branch/fork orchestration, import preview/commit orchestration, approval request/decision orchestration, merge-context construction/secret detection, temporary preview generation, and thread snapshot/status normalization are isolated in dedicated helpers; session eviction/capacity rules and housekeeping cleanup are isolated in dedicated maintenance/policy helpers; Codex RPC bootstrap is centralized in runtime helpers; notification side effects are isolated in a dedicated helper service; and session-exit recovery flow is isolated in a dedicated recovery helper, so transcript/history/session contracts stay stable while session orchestration remains focused.
